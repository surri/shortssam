...
function toWork(data: GenResult, thumb: string, persona?: string): Work {
  return {
...
export async function POST(req: Request) {
  ...
  const { image, thumb, presetKey, persona } = parsed.data
  ...
  if (presetKey) {
    ...
    return NextResponse.json(toWork(mock, "", persona))
  }

  try {
    const data = await generateScript(image!, persona)
    const work = toWork(data, thumb || image!, persona)
    ...
    const fallback = toWork(MOCK_WORKS.quad, thumb || "", persona)
